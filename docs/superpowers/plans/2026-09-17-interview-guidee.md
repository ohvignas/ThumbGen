...
23. **(Added during execution) Strictly increasing `updated_at`.** A save and a placement in the same millisecond would give `baseUpdatedAt === placedByAgentAt` and the save would erase a node the client never saw. `nextUpdatedAt(previous)` (`canvas-patch.ts`) returns now, or 1 ms after the stored value when that is not in the past; `saveProject` and `placeInterviewNode` use it inside their transactions.
24. **(Added during execution) Reinjected edges.** Only stored edges that touch a reinjected node are reinjected (both ends present, not already in the payload): an edge Antoine removed between two nodes he kept is never resurrected.
25. **(Added during execution) `place_node` input schema.** `z.object({ node: z.looseObject({ id, type, data? }) })` without `z.preprocess` (the AI SDK converts the schema to JSON Schema); `normalizeNode` runs inside `placeInterviewNode`. `createProject` ids (`proj_<Date.now()>`) collide within a millisecond, so the new DB tests insert uuid project ids.
