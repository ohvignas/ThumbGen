...
- An iterate of an already-generated thumb (same angle): pass that image as `reference_sources: ["stored:gi_<id>"]` and write a **short edit prompt** (not 7 sentences). A/B first-gen sketches still use the full anatomy.
...
- **MCP / no-brief chats have no sketch-node cap**, but still cost money. Stay stingy.
...
| `face_source` | no | **`stored:persona_<id>`** when the creator's face is in the frame. The tool loads the Personnage **front** angle (else first available). Attached **first**. Prompt lock: IDENTITY / AVATAR, keep this face, do not invent a new head. Without it, you get a generic stranger. |
...
**SCRATCH / FIRST GEN** — at most **7 sentences**, this order, **no labels** in the prompt. Omit a line if irrelevant. Full rubric lives in `thumbnail-packaging` / the system block. First-gen A/B = one full anatomy per variant, not short prompts.

**ITERATE / ADJUST THIS IMAGE** — `reference_sources` includes `stored:gi_<id>` (or they asked to retouch a generated thumb). Same angle. Write **1–3 sentences**: the change first, then what to keep. Keep the chain: original prompt + this generation + the requested change. Do not rebuild SUBJECT/SCENE/COMPOSITION. The tool sends that generated image first as the edit source. Short because you are iterating, not because A/B is on.

From-scratch bar:

1. **SUBJECT** — If `face_source` is set: **"the person in the identity/avatar reference photos"** plus pose and decomposed face (not "shocked"). Never a generic "Young man". Outfit if it matters.
...
Do **not** put in the prompt: ALL-CAPS emphasis, "preserve face fidelity" (the tool appends the IDENTITY / AVATAR lock when `face_source` is set), CTR/viral marketing, 8K/masterpiece piles, invalid framings (`ultra close-up`). Medium shot has no room for a giant logo beside a torso — use wide/full if subject left + logo right.
...
When this conversation has a brief, the server checks **before** the HTTP call:

`limit = 2 × max(1, variants.length) + 3` **live** `type: "sketch"` nodes on the current canvas (`<canvas_state>.liveSketchCount` / `get_canvas_state.liveSketchCount`).

Examples: 0 or 1 variant → 5 live nodes; two → 7; three → 9. No brief (plain canvas / MCP) → no cap.

**Only visible sketch nodes count.** Deleted / tombstoned nodes and earlier `generate_sketch` calls that are not on the canvas do **not** count. One live croquis plus four deleted ones is **1**, not 5 — you may sketch. Never tell the user a 5-per-project or "session including deleted" quota is full unless the tool just returned the refusal below **and** `liveSketchCount` already equals that limit.

Past the live-node limit the handler is **not** called. Quote the refusal and stop sketching. Otherwise call the tool — do not refuse yourself because you remember older drafts.

A reservation is **refunded** only when `requestNotSent` is true (key missing, unresolved ref, network throw — the paid request never left). Provider errors, empty image, and unknown throws **stay booked** as cost, not as canvas nodes. Do not retry those hoping for a free extra.
...
4. `generate_sketch` (one call per variant you actually show). Parallel calls each cost ~$0.02. The cap is live canvas sketch nodes, not how many times you called the tool.
5. Show it: `finish_turn` with that `result_id`, short summary, ask_agent ("Garder A", "Retoucher le fond") — **never** an ask_agent that would start a paid generation.
6. When they validate: `apply_workflow` / `place_node` (sketch node on `sketch-in`, Personnage on `face-in`, logos on `logo-in`) then `finish_turn` kind `generate` so **they** click **Générer**.
