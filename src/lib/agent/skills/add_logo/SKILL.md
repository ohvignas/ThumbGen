---
name: add_logo
description: Saves one find_logos candidate into the library as stored:lg_<id>. Use after the user picked a logo-candidate id or a single obvious hit. Pass candidate_id only, never a URL. Max 3 logos per conversation. Unknown id is refused. Not for a user file or library pick — that is request_user_image.
---

# add_logo

Persists **one** `find_logos` hit into the **Logos** table. Chat label: « Enregistre le logo ». Free: no OpenRouter, no YouTube quota, no Brandfetch fetch.
...
- Several selected ids (max 3 **kept** logos total this conversation) — one call per id. Parallel in the same step is fine.

## When not

- Brand **already** in `list_logos` / the canvas as `stored:lg_<id>` — reuse that ref. Do not search-and-add a second copy.
...
- This conversation already has **3** logos — ask which to drop. Do not call. A fourth attempt still downloads then fails `"3 logos maximum."` (orphan row in the library).
...
- Wiring `logo-candidate:` onto a node or `generate_sketch.reference_sources`. Only `stored:lg_<id>` is valid (`^stored:lg_[\w-]+$`).
- Do not re-call the same `candidate_id`. Each success **inserts a new uuid** (not idempotent). A second add is a **new** row.
...
6. `appendBriefLogo({ name: candidate.name, source: "stored:lg_<uuid>" })`:
   ...
   - Same `source` already stored this conversation → success, no duplicate.
...
| Quoted text | Meaning | What you do |
|---|---|---|
| `3 logos maximum.` | This conversation already has 3, or the append could not be stored | Ask which logo to drop. Do not add a fourth. |
...
3. Success → optional `place_node` / `apply_workflow` swipeFile **kind=logo** on **logo-in**; optional `generate_sketch` `reference_sources`.
