...
import { createFakeAgentModel, fakeAgentScenario, isFakeAgentEnabled } from "./fake-agent-model";
...
export function resolveAgentLanguageModel(modelId: string): ResolvedAgentModel | null {
  ...
  if (isFakeAgentEnabled()) return { model: createFakeAgentModel({ scenario: fakeAgentScenario() }), fake: true };
